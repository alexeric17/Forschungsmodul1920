...
object RuntimeTest {
  def main(args: Array[String]): Unit = {
    ...
    val vertices_10k = graph_10k.vertices.collect().toList.map(r => r.asInstanceOf[VertexId])

    val graph_50k = get_subgraph(nodesDF, edgeDF, 50000, take_highest = true)
    val vertices_50k = graph_50k.vertices.collect().toList.map(r => r.asInstanceOf[VertexId])
